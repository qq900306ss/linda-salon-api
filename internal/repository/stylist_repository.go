...
func (r *StylistRepository) GetTopStylists(limit int, startDate, endDate time.Time) ([]map[string]interface{}, error) {
	...
	err := r.db.Model(&model.Stylist{}).
		Select("stylists.id, stylists.name, COUNT(bookings.id) as booking_count, SUM(bookings.price) as revenue").
		Joins("LEFT JOIN bookings ON bookings.stylist_id = stylists.id AND bookings.status = ? AND bookings.booking_date BETWEEN ? AND ?",
			model.BookingStatusCompleted, startDate, endDate).
